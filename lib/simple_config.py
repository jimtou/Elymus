...
SYSTEM_CONFIG_PATH = "/etc/elymus.conf"
...
def read_system_config(path=SYSTEM_CONFIG_PATH):
    """Parse and return the system config settings in /etc/elymus.conf."""
    result = {}
...
def read_user_config(path):
    """Parse and store the user config settings in elymus.conf into user_config[]."""
    if not path:
        return {}
